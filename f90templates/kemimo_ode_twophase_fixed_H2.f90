...
contains
  ...
  subroutine dochem(n,dt)
    ...
    neqa(:) = nmols
    ! set error per step size correctio array
    ewt_fac(:) = 1.0d0
    !  FROM DLSODES manual
  ...
  subroutine fex(neq, tt, n, dn)
    ...
    dn(idx_surface_mask) = dn(idx_surface_mask) * kall(nrea) + 1d-40 ! adding small offset for convergence
  ...
  subroutine jes(neq, tt, n, j, ian, jan, pdj)
    ...
    pdj(idx_surface_mask) = pdj(idx_surface_mask) * kall(nrea)  + 1d-40
