module kemimo_ode
  ! Experimental fast path for a dynamically insignificant mantle.
...
contains
  ...
  subroutine dochem(n,dt)
    ...
    ! Divide the tolerance budget over all mantle species.
    ...
    do
       ...
       elseif(istate==-3) then
          ...
          n(:) = ni(:)
          dt = dt / 5d0
          kall(:) = kall(:) + 1d-40 ! small budge to aid convergence
          istate = 1
       else
  ...
  subroutine jes(neq, tt, n, j, ian, jan, pdj)
    ...
    do i=1, surface_end
      ...
      if ((ewt_fac(i) < 1d0) .or. (ewt_fac(i) /= ewt_fac(i))) ewt_fac(i) = 1d0
    enddo
    ! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    !!END_JACOBIAN
