...
contains
  ...
  function fluxesDust(n, idxList) result(flux)
      ...
      do i=1, nrea-1
         ...
         if (layer == 1) then
          if (rtype == 5) flux(i) = flux(i) * 1d0/max(1d0, min(n(idx_surface_mask), real(layerThickness)))
          if (rtype == 4) flux(i) = flux(i) * min(n(idx_surface_mask), 4d0)
          if (rtype == 3) flux(i) = flux(i) * min(n(idx_surface_mask), 1d0)
         endif
