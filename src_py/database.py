...
class database:
    ...
    def loadKIDA(self, fileName="gasNetwork.dat"):
        ...
        for row in open(self.datadir + fileName, "rb"):
            ...
            if rea.formula not in [0, 1, 2, 3, 4, 5, 11]:
                continue
    ...
    def preproc(self):
        ...
        if self.H2spin:
            copyfile('./f90templates/kemimo_rates.f90', './kemimo_rates.f90')
        else:
            copyfile('./f90templates/kemimo_rates_nospin.f90', './kemimo_rates.f90')
    ...
    def checkBranchingRatios(self, reactions):
        ...
        for i, rea in enumerate(reactions[:]):
            ...
            if reactants in reactants_list:
                ...
                if any(pp.name.startswith('o_') for pp in rea.products):
                    count[idx] += 0
                elif any(pp.isGas for pp in rea.products):
                    count[idx] += 0
                else:
                    count[idx] += 1
            else:
        ...
        for i, idx in enumerate(indices):
            if count[i] > 1:
                ratio = 1.0 / count[i]
                for ii in idx:
                    print("adding Bratio to: ", reactions[ii].verbatim, ratio)
                    reactions[ii].Bratio = ratio
                    reactions[ii].bRatioFromFile = False
    ...
    def saveNetworkToFile(self, fileName="network.ntw"):
        ...
        fout.write(tabrow(["#type", "Bratio", "Bratio from file", "Ea/K", "R", "R",
                           "P", "P"], lmax=18).strip() + "\n")
        # loop on reactions
        for rea in self.reactions:
            ...
            fout.write(tabrow([rea.type, rea.Bratio, rea.bRatioFromFile, rea.Ea] + reactantsName
    ...
    def getSticking(self):
        ...
        E_lc = {'H2': 315.0, 'HD': 650.0, 'D2': 650.0, 'N2': 1250.0, 'CO': 1480.0, 'O2': 1310.0, 'CH4': 1440.0, 'CO2': 2320.0}
